...
class CMFlow_T(nn.Module):
    ...
    def __init__(self,args):
        ...
        self.agent_cross_attention =AgentAttention(input_dim=3,dim=128, num_heads=8, agent_num=49, attn_drop=0.1, proj_drop=0.1)
    ...
    def Backbone(self,pc1,pc2,feature1,feature2,gfeat_prev,vel1,vel2,mask1,mask2):
        ...
        cor_features,rigid_velocities = self.fc_layer(pc1, pc2, pc1_features, pc2_features,vel1,vel2,mask1,mask2,generator=self.generator,gfeat=gfeat_prev)
        ## generate embeddings
        embeddings = torch.cat((feature1, pc1_features, cor_features,rigid_velocities.permute(0,2,1)),dim=1)
        ...
        return final_features, gfeat_new,rigid_velocities
    ...
    def forward(self, pc1, pc2, feature1, feature2, label_m, mode, gfeat,vel1,vel2,mask1,mask2,v_train):
        
        # extract backbone features 
        final_features, gfeat,rigid_velocities = self.Backbone(pc1,pc2,feature1,feature2, gfeat,vel1,vel2,mask1,mask2)
        if v_train:
            return rigid_velocities
        print(f'刚性速度的shape是{rigid_velocities.shape}')
        # predict initial scene flow and classfication map
        output = self.fp(final_features,rigid_velocities)
        print(f'output的shape是{output.shape}')
        stat_cls = self.mp(final_features)
        output_transposed = output.permute(0, 2, 1)  # Now output has shape [B, 256, 3]
        output=self.agent_cross_attention(output_transposed, rigid_velocities, rigid_velocities)
        print(output.shape)
        ...
        return sf_agg, stat_cls, pre_trans, mask, gfeat,rigid_velocities
